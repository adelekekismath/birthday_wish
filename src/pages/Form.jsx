...
function Form() {
  ...
  useEffect(() => {
    if (location.state && location.state.wish) {
      ...
      setFormData({
        ...
        birthdayMessage: wish.birthdayMessage,
        theme: wish.theme,
        senderEmail: wish.senderEmail,
        favoriteColor: wish.favoriteColor,
      });
      setSelectedImageUrl(wish.celebrantPhotoUrl);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const wishId = location.state?.wish?.id;
      console.log(wishId)
      if (wishId) {
        const docRef = doc(db, "birthdayWishes", wishId);
